...
def constraint_and_falloff(defaultV=0.5,driver_typ=None,output=True):
    ...
    if 'pointConstraint'in driver_typ:
        ...
        cmds.orientConstraint(driverA,driverB, driven, maintainOffset=True,weight = defaultV)
    ...
    cmds.connectAttr(f"{reverse}.outputX", f"{driven}_{driver_typ}1.{driverB}W1", force=True)
    cmds.connectAttr(f'{driven_ctrl}.fallOff', f"{driven}_{driver_typ}1.{driverA}W0", force=True)
...
def create_zip_lip(left_ctrl=None,right_ctrl=None,jaw_ctrl=None,zip_height=0.5,fallOff=3):
    ...
    for upper_jnt,downer_jnt in zip(upper_jnts,downer_jnts):
        ...
        #blend lip joint 
        parent_con_up = cmds.parentConstraint(zip_loc,driven_jnt,upper_jnt, maintainOffset=False)[0]
...
def attach_target_on_uvPin(curve=None, curve_up=None):
    '''
    Select targets first

    curve(str)    : main curve
    curve_up(str) : up curve (rail)
    '''
    ...
    if not sels:
        ...
        return  
    sels = cmds.listRelatives(sels,children=True)
    curve_shape = cmds.listRelatives(curve, s=True)[0]
    curve_up_shape = cmds.listRelatives(curve_up, s=True)[0]

    uvPin = cmds.createNode('uvPin', name=curve.replace('curve','uvPin'))

    cmds.connectAttr(f'{curve_shape}.worldSpace[0]', f'{uvPin}.deformedGeometry', force=True)
    cmds.connectAttr(f'{curve_up_shape}.worldSpace[0]', f'{uvPin}.railCurve', force=True)

    cmds.setAttr(f'{uvPin}.tangentAxis', 1)   # Y
    cmds.setAttr(f'{uvPin}.normalAxis', 2)    # Z
    cmds.setAttr(f'{uvPin}.normalOverride', 1)

    for i, sel in enumerate(sels):

        pos = cmds.xform(sel, q=True, ws=True, t=True)
        param = get_param_on_curve(curve, pos)
        cmds.setAttr(f'{uvPin}.coordinate[{i}].coordinateU', param)

        dcm = cmds.createNode('decomposeMatrix', name=f'{sel}_dcm')

        cmds.connectAttr(
        f'{uvPin}.outputMatrix[{i}]',
        f'{dcm}.inputMatrix',
        force=True
        )

        cmds.connectAttr(f'{dcm}.outputTranslate', f'{sel}.translate', force=True)
        cmds.connectAttr(f'{dcm}.outputRotate', f'{sel}.rotate', force=True)

    return uvPin


def create_joint_on_vertex(name_prefix="joint"):
    """
    Create joints at selected vertices
    
    Args:
        name_prefix (str): prefix for joint naming
    ...
    joints = []
    grp = cmds.createNode('transform',name=f'{name_prefix}_grp')
    i = i+1
    for i, vtx in enumerate(verts):
        ...
        # create joint
        jnt = cmds.joint(name=f"{name_prefix}_{i:04d}", position=pos)
